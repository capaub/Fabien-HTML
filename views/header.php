...
<header>
    <nav>
        ...
        <a href="?page=<?= PAGE_CONTACT; ?>">Me contacter</a>
        <?php if(isset($_SESSION['user']) && $_SESSION['user'] instanceof User) { ?>
            <?php if ($_SESSION['user']->getRole() === User::ROLE_ADMIN): ?>
                <a href="?page=<?= PAGE_ADMIN?>">Espace admin</a>
            <?php endif; ?>
            <a href="?page=<?= PAGE_MY_ACCOUNT?>">Mon compte(<?= $_SESSION['user']->getUsername() ?></a>
            <a href="?page=<?= PAGE_LOGOUT?>">Me déconnecter</a>
        <?php } else { ?>
            <a href="?page=<?= PAGE_SIGNUP?>">Inscription</a>
